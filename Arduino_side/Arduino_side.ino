...
const float prob = 0.3;            //角度データの信頼度[%]（0～1の値 ローパスフィルタで使用）
...
void setup()
{
  Serial.begin(115200);  //加速度センサとESP32とのシリアル通信
  Serial1.begin(115200); //深度センサとのシリアル通信
  Wire.begin();
...
void loop()
{
  ...
  float depth_conf = ping.confidence(); //深さの信頼度取得(ライブラリ)
  ...
  Serial.println("角度センサX:" + String(x_ang) + "," + "制御波形X:" + String(target_deg_x) + "," + "角度センサY:" + String(y_ang) + "," + "制御波形Y:" + String(y_ctl + 90) + "," + "深さセンサ:" + String(depth) + "," + "深さ制御:" + String(depth_conf));
