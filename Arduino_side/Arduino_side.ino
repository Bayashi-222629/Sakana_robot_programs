...
#define RD_R_V A2 // R:right,L:left,B:beside,V:vertical
#define RD_L_B A3
...
void loop()
{
  ...
  float rd_R_B = get_data_R_B(); //レシーバから値を受信し、フィルタに通してdegreeに変換(get_filtered_data.ino)
  float rd_L_V = get_data_L_V(); // L_B＝ロール、L_V＝ピッチ、R_V＝メインモータ回転速度、R_B＝割り当てなし
  float rd_R_V = get_data_R_V();
  float rd_L_B = get_data_L_B();
