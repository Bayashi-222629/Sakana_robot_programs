...
float get_data_z()
{
    z_data = ac.getCalculatedZ();
    filtered_z_data = z_data * prob + pre_filtered_z_data * (1 - prob);
...
float get_data_x()
{
    x_data = ac.getCalculatedX();                                       //角度センサから値を読み取る(ライブラリ)
    filtered_x_data = x_data * prob + pre_filtered_x_data * (1 - prob); //ローパスフィルタの再現
    pre_filtered_x_data = filtered_x_data;

    float deg = atan2(filtered_x_data, filtered_z_data) * 180.0 / PI;
...
float get_data_y()
{
    ...
    float deg = atan2(filtered_y_data, filtered_z_data) * 180.0 / PI;
