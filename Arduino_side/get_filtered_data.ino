...
float rd_R_B_data, pre_rd_R_B = 2.5;
float rd_L_V_data, pre_rd_L_V = 2.5;
float rd_R_V_data, pre_rd_R_V = 2.5;
float rd_L_B_data, pre_rd_L_B = 2.5;
...
const int voltage = 5;
const int deg_noise = 200; // これ以上のプロポの入力はノイズとして弾く

float deg;

float get_data_x() // 加速度センサX
{
...
float get_data_R_B() // プロポ入力右横
{
    ...
    else if (deg > deg_max && deg < deg_noise)
    {
        deg = deg_max;
    }
    else if (deg < deg_min)
